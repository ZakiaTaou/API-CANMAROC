...
// ==============================
 export const validateTeam = [
  ...
  body('coach')
    .trim()
    .notEmpty().withMessage('Le nom de l\'entraîneur est requis'),
  
  body('group')
    .trim()
    .notEmpty().withMessage('Le groupe est requis')
    .isIn(['A', 'B', 'C', 'D', 'E', 'F']).withMessage('Le groupe doit être A, B, C, D, E ou F'),
  
  body('flag_url')
    .optional()
    .isURL().withMessage('L\'URL du drapeau doit être valide')
];
...
export const validateMatch = [
  ...
  body('teamawayid')
    .notEmpty().withMessage('L\'équipe extérieure est requise')
    .isInt().withMessage('L\'ID doit être un nombre')
    .custom((value, { req }) => {
  ...
  body('match_date')
    .notEmpty().withMessage('La date du match est requise')
    .isISO8601().withMessage('La date doit être au format ISO8601 (YYYY-MM-DD)'),
  
  body('stadium')
    .trim()
    .notEmpty().withMessage('Le stade est requis')
    .isLength({ min: 3, max: 100 }).withMessage('Le nom du stade doit contenir entre 3 et 100 caractères'),
  
  body('status')
    .optional()
    .isIn(['scheduled', 'live', 'finished']).withMessage('Le status doit être: scheduled, live ou finished'),
  
  body('score_home')
    .optional()
    .isInt({ min: 0 }).withMessage('Le score domicile doit être un nombre positif'),
